...
def build_anomaly_prompt(stats: dict) -> str:
    ...
    top_suspects = stats.get(
        "top_suspects", []
    )  # liste de dicts {ip, nb_connexions, nb_ports_distincts, ratio_deny, profil, anomaly_score}
...
def build_classification_prompt(stats: dict) -> str:
    ...
    feat_lines = "\n".join(
        [f"  • {f} : {s:.4f}" for f, s in list(importance.items())[:7]]
    )
...
def build_temporal_prompt(stats: dict) -> str:
    ...
    permit_lines = "\n".join(
        [f"  • {h}h : {c:,} PERMIT" for h, c in permit_by_hour.items()]
    )
    pics_lines = "\n".join(
        [
            f"  • {p['horodatage']} — {p['count']:,} cnx — z={p['zscore']:.2f}"
            for p in pics_details[:5]
        ]
    )
    profil_lines = "\n".join(
        [f"  • {prof} → pic à {h}h" for prof, h in profil_hours.items()]
    )
...
def build_global_threat_prompt(df_deny, geo_cache: dict) -> str:
    ...
    port_lines = [
        f"  • Port {p} ({_port_name(p)}) — {c} tentatives" for p, c in top_ports.items()
    ]
    proto_str = (
...
def _fallback_classification(stats: dict) -> str:
    ...
    quality = (
        "excellent" if accuracy > 0.9 else "bon" if accuracy > 0.75 else "acceptable"
    )
...
Cite les IPs réelles. 2-3 phrases par section. Pas de contenu offensif."""
        ...
        ext_lines = "\n".join(
            [f"  • {r['ip']} — {r['connexions']:,} cnx" for r in top_ext[:8]]
        )
...
Sois précis, cite les données réelles. 2-3 phrases par section. Pas de contenu offensif."""
        ...
        corr_lines = "\n".join(
            [f"  • {r['feat_a']} ↔ {r['feat_b']} : r={r['corr']:.3f}" for r in top_corr]
        )
...
def _fallback_behavior(tab: str, stats: dict) -> str:
    ...
    if tab == "src":
        ...
        lines = "\n".join(
            [
                f"- **{r['ip']}** : {r['connexions']:,} connexions — {r['profil']}"
                for r in top5
            ]
        )
...
*(Rapport de secours — ajoutez une clé Mistral pour l'analyse IA)*"""
        ...
        plines = "\n".join(
            [
                f"- Port **{r['port']}** ({r['service']}) : {r['count']:,} accès"
                for r in top_ports[:5]
            ]
        )
