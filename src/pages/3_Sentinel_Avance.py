...
def _train_random_forest(ipf: pd.DataFrame):
    ...
    cv_scores = cross_val_score(
        rf, X, y, cv=5, scoring="roc_auc_ovr_weighted", n_jobs=-1
    )
...
with tab_anomaly:
    ...
    if st.button("🔬 Lancer la détection d'anomalies"):
        with st.spinner("Feature engineering & Isolation Forest…"):
            ...
            ip_feat["ratio_sensible"] = ip_feat["nb_ports_sensibles"] / ip_feat[
                "nb_connexions"
            ].clip(lower=1)
...
with tab_classif:
    ...
    if ipf is None:
        st.info(
            "💡 Lancez d'abord **Détection d'anomalies** pour générer les features IP."
        )
        st.markdown(
        ...
        if result is None:
            ...
            with col_cm:
                ...
                st.caption(
                    "Chaque cellule indique combien d'IPs du profil réel (ligne) ont été classées dans le profil prédit (colonne). La diagonale = bonnes prédictions."
                )
                fig_cm = px.imshow(
                    cm,
                    x=list(cls),
            ...
            with col_feat:
                ...
                st.caption(
                    "Score d'importance Gini : contribution moyenne de chaque feature à la réduction d'impureté dans les 200 arbres. Plus le score est élevé, plus la feature est discriminante."
                )
                colors_f = [
            ...
            if result["roc"]:
                ...
                st.caption(
                    "Chaque courbe mesure la capacité du modèle à détecter un profil (TPR) sans générer de fausses alarmes (FPR). AUC = 1.0 → classification parfaite · AUC = 0.5 → aléatoire."
                )
                if len(ipf) < 500:
                    st.warning(
                ...
                fig_r = go.Figure()
                for i, (c_name, fpr, tpr, roc_auc) in enumerate(result["roc"]):
                    fig_r.add_trace(
                        go.Scatter(
                            x=fpr,
                            y=tpr,
            ...
            st.caption(
                "Précision = parmi les IPs classées dans ce profil, combien le sont vraiment · Rappel = parmi les vraies IPs de ce profil, combien sont détectées · F1 = moyenne harmonique (score vert > 0.8, orange > 0.5, rouge ≤ 0.5)."
            )
            rows_m = ""
            for c_name in cls:
                d = rep.get(c_name, {})
...
with tab_behavior:
    ...
    if ipf_b is None:
        st.info(
            "💡 Lancez d'abord **🔬 Détection d'anomalies** pour calculer les features comportementales."
        )
        st.stop()
    ...
    with bt1:
        ...
        with sc1:
            st.markdown(
                "<div class='section-hd'>Connexions vs Ports distincts — taille = ratio DENY</div>",
                unsafe_allow_html=True,
            )
            fig_s1 = px.scatter(
                ip_sel,
        ...
        with sc2:
            st.markdown(
                "<div class='section-hd'>Activité nocturne vs Ratio DENY — taille = volume</div>",
                unsafe_allow_html=True,
            )
            fig_s2 = px.scatter(
                ip_sel,
            ...
            fig_s2.update_layout(
                ...
                legend=dict(bgcolor="rgba(0,0,0,0)", font_size=10),
                xaxis=dict(
                    gridcolor="#1e2a38", title="% activité 0h–6h", tickformat=".0%"
                ),
        ...
        # Top 5 IP sources
        st.markdown(
            "<div class='section-hd'>🖥 Top 5 IP sources les plus émettrices</div>",
            unsafe_allow_html=True,
        )
        ...
        src_stats = {
            ...
            "top5_src": [
                {
                    "ip": r["ip_src"],
                    "connexions": int(r["nb_connexions"]),
                    "ratio_deny": (
                        float(r["ratio_deny"]) if pd.notna(r["ratio_deny"]) else 0
                    ),
                    "profil": str(r.get("profil", "Normal")),
                }
                for _, r in top5_src.iterrows()
            ],
            "profil_dist": {
                k: int(v) for k, v in ipf_b["profil"].value_counts().items()
            },
        ...
        render_ai_panel(
            ...
            generate_fn=lambda key, model: generate_analysis(
                "behavior", key, model, stats=src_stats
            ),
            requires_key=False,
        )
    ...
    with bt2:
        ...
        dst_agg = (
            ...
            .reset_index()
            .sort_values("nb_connexions", ascending=False)
        )

        st.markdown(
            "<div class='section-hd'>Top 10 IP destinations — PERMIT vs DENY</div>",
            unsafe_allow_html=True,
        )
        ...
        with dc1:
            st.markdown(
                "<div class='section-hd'>Table détaillée des destinations (Top 20)</div>",
                unsafe_allow_html=True,
            )
            st.dataframe(
                dst_agg.head(20)[
                ...
                .rename(
                    columns={
                        ...
                        "nb_src_uniques": "Sources uniques",
                        "ratio_deny": "Ratio DENY",
                        "ports_uniques": "Ports",
                        "nb_permit": "PERMIT",
                        "nb_deny": "DENY",
                    }
        ...
        with dc2:
            st.markdown(
                "<div class='section-hd'>🌐 Accès depuis IPs hors RFC 1918</div>",
                unsafe_allow_html=True,
            )
            ext_src = (
                df_raw[~df_raw["ip_src"].apply(is_private_ip)]["ip_src"]
                .value_counts()
                .head(15)
                .reset_index()
            )
        ...
        dst_stats = {
            ...
            "top_ext": (
                [
                    {"ip": r["ip_ext"], "connexions": int(r["nb_connexions"])}
                    for _, r in ext_src.head(8).iterrows()
                ]
                if not ext_src.empty
                else []
            ),
        ...
        render_ai_panel(
            ...
            generate_fn=lambda key, model: generate_analysis(
                "behavior", key, model, stats=dst_stats
            ),
            requires_key=False,
        )
    ...
    with bt3:
        ...
        n_att = st.slider(
            "Nombre d'attaquants à afficher", 5, 30, 15, key="beh_top_slider"
        )
        suspects_b = (
            ipf_b[ipf_b["profil"] != "Normal"].sort_values("anomaly_score").head(n_att)
        )

        st.markdown(
            "<div class='section-hd'>Carte comportementale des attaquants</div>",
            unsafe_allow_html=True,
        )
        fig_att = px.scatter(
            suspects_b,
        ...
        fig_att.update_traces(
            textposition="top center", textfont=dict(size=8, color="#c8d8e8")
        )
        ...
        st.dataframe(
            ...
            .rename(
                columns={
                    "ip_src": "IP",
                    "nb_connexions": "Connexions",
                    "nb_ports_distincts": "Ports",
                    "ratio_deny": "Ratio DENY",
                    "nb_ports_sensibles": "Ports sensibles",
                    "activite_nuit": "Activité nuit",
                    "profil": "Profil",
                    "anomaly_score": "Score",
                }
            )
        ...
        st.markdown(
            "<div class='section-hd'>🔑 Top 10 ports sensibles (&lt;1024) avec accès PERMIT</div>",
            unsafe_allow_html=True,
        )
        df_permit_b = (
            df_raw[df_raw["action"] == "PERMIT"]
            if "action" in df_raw.columns
            else pd.DataFrame()
        )
        if not df_permit_b.empty and "port_dst" in df_permit_b.columns:
            top_perm_ports = (
                df_permit_b[df_permit_b["port_dst"] < 1024]["port_dst"]
                .value_counts()
                .head(10)
                .reset_index()
            )
            top_perm_ports.columns = ["port", "count"]
            top_perm_ports["service"] = top_perm_ports["port"].map(
                lambda p: PORT_NAMES_B.get(int(p), "Inconnu")
            )
            top_perm_ports["risk"] = top_perm_ports["port"].map(
                lambda p: (
                    "🔴 Critique"
                    if int(p) in [22, 23, 3389, 445, 1433, 3306, 5900]
                    else (
                        "🟡 Modéré" if int(p) in [80, 443, 21, 25, 53] else "⚪ Faible"
                    )
                )
            )
            ...
            with tc2:
                colors_pp = [
                    (
                        "#ff3c6e"
                        if int(r["port"]) in [22, 23, 3389, 445, 1433, 3306, 5900]
                        else (
                            "#ffb800"
                            if int(r["port"]) in [80, 443, 21, 25, 53]
                            else "#00d4ff"
                        )
                    )
                    for _, r in top_perm_ports.iterrows()
                ...
                fig_pp = go.Figure(
                    go.Bar(
                        ...
                        y=[
                            f":{int(r['port'])} {r['service']}"
                            for _, r in top_perm_ports.iterrows()
                        ],
                        orientation="h",
                        marker_color=colors_pp,
                        opacity=0.9,
                    )
                )
            ...
            "top_attackers": [
                {
                    ...
                    "connexions": int(r["nb_connexions"]),
                    "ports": int(r["nb_ports_distincts"]),
                    "ratio_deny": float(r["ratio_deny"]),
                    "profil": r["profil"],
                }
        ...
        render_ai_panel(
            ...
            generate_fn=lambda key, model: generate_analysis(
                "behavior", key, model, stats=att_stats
            ),
            requires_key=False,
        )
    ...
    with bt4:
        ...
        feat_available = [
            f for f in FEAT_COLS + ["anomaly_score"] if f in ipf_b.columns
        ]
        corr = ipf_b[feat_available].corr()

        st.markdown(
            "<div class='section-hd'>Matrice de corrélation des features comportementales</div>",
            unsafe_allow_html=True,
        )
        fig_corr = px.imshow(
            corr,
        ...
        # Radar chart
        st.markdown(
            "<div class='section-hd'>Radar — Empreinte comportementale normalisée par profil</div>",
            unsafe_allow_html=True,
        )
        radar_cats = [
            f
        ...
        if "datetime" in df_raw.columns and not df_raw["datetime"].isna().all():
            st.markdown(
                "<div class='section-hd'>Évolution temporelle des DENY par protocole (granularité 6h)</div>",
                unsafe_allow_html=True,
            )
            pcol_b = (
                "protocol_clean" if "protocol_clean" in df_raw.columns else "protocol"
            )
        ...
        for i, fa in enumerate(corr.columns):
            for j, fb in enumerate(corr.columns):
                if j > i and abs(corr.iloc[i, j]) > 0.2:
                    corr_flat.append(
                        {"feat_a": fa, "feat_b": fb, "corr": float(corr.iloc[i, j])}
                    )
        ...
        render_ai_panel(
            ...
            generate_fn=lambda key, model: generate_analysis(
                "behavior", key, model, stats=corr_stats
            ),
            requires_key=False,
        )
