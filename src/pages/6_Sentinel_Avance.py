...
with st.sidebar:
    ...
    map_style = st.selectbox(
        "🗺 Style de carte",
        ["Dark Matter (sombre)", "Voyager (colorée)", "Positron (claire)"],
    )
...
with k2:
    neon_metric(
        "🚫 DENY", f"{int((df_raw['action']=='DENY').sum()):,}", color="var(--accent2)"
    )
with k3:
    neon_metric(
        "✅ PERMIT",
        f"{int((df_raw['action']=='PERMIT').sum()):,}",
        color="var(--accent4)",
    )
with k4:
    neon_metric(
        "🖥 Sources uniques", f"{df_raw['ip_src'].nunique():,}", color="var(--accent3)"
    )
with k5:
    neon_metric(
        "🎯 Destinations", f"{df_raw['ip_dst'].nunique():,}", color="var(--accent3)"
    )
...
with tab_map:
            ...
            st.pydeck_chart(
                pdk.Deck(
                    ...
                    initial_view_state=pdk.ViewState(
                        latitude=25, longitude=10, zoom=1.4, pitch=map_pitch
                    ),
                    map_style=MAP_STYLES[map_style],
                    tooltip={
                        "html": """<div style='font-family:monospace;font-size:11px;background:#0d1117;
    ...
    with feed_col:
        ...
        if st.session_state.flow_log:
            ...
            st.markdown(
                f"<div class='kpi-row'><span class='kpi-chip deny'>🚫 {deny_n}</span><span class='kpi-chip ok'>✅ {permit_n}</span></div>",
                unsafe_allow_html=True,
            )
            ...
            st.markdown(
                """<div style='background:var(--bg2);border:1px dashed var(--border);border-radius:6px;
    ...
    if st.session_state.top_src_df is not None:
        ...
        with a1:
            st.markdown(
                "<div class='section-hd'>Top IPs sources</div>", unsafe_allow_html=True
            )
            st.dataframe(
                st.session_state.top_src_df,
                use_container_width=True,
        ...
        if st.session_state.country_src is not None:
            ...
            for col, data, color, title in [
                ...
                (
                    ...
                    st.session_state.country_dst,
                    "#00d4ff",
                    "Pays destinations — Top 10",
                ),
            ]:
                with col:
                    st.markdown(
                        f"<div class='section-hd'>{title}</div>", unsafe_allow_html=True
                    )
                    fig = px.bar(
                        x=data.values,
                        y=data.index,
